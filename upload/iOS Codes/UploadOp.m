...
// Posting constants
#define IMAGE_CONTENT @"Content-Disposition: form-data; name=\"%@\"; filename=\"image1.jpg\"\r\nContent-Type: image/jpeg\r\n\r\n"
...
- (NSString *)uploading
{
	...
	[postParam setObject:imageData forKey:@"uploader"];
//	[postParam setObject:self.email forKey:@"email"];
    
	NSData *postData = [self generateFormDataFromPostParam:postParam];
    NSString *postLength = [NSString stringWithFormat:@"%d", postData.length];

    NSString *urlString = [NSString stringWithFormat:@"%@%@",@"http://192.168.254.97/~zzl/PHP/CodeSnippet/upload/", @"upload_file.php"];
